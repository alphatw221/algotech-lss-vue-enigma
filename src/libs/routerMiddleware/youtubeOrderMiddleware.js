...
import { guest_retrieve_order_platform } from '@/api_v2/order'
import { guest_retrieve_pre_order_platform } from '@/api_v2/pre_order'
...
export default async (to, from)=>{
    ...
    if(order_oid){
        ...
        object_id=order_oid
        try{
            res = await guest_retrieve_order_platform(order_oid)
        }catch(error){}
    }else if(pre_order_oid){
        type = 'cart'
        object_id=pre_order_oid
        try{
            res = await guest_retrieve_pre_order_platform(pre_order_oid)
        }catch(error){}
    }
