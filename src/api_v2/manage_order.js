...
export const manage_order_list = (campaign_id,searchValue,page,page_size,status,data) => {
    return createAxiosWithBearer().post(`/api/v2/manage-order/order_list/?campaign_id=${campaign_id}&search=${searchValue}&page=${page}&page_size=${page_size}&status=${status}`,data)
}