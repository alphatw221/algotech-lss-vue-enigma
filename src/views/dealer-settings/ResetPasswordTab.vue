<template> 
    <div  class="intro-y py-5 text-[16px] sm:text-[1.2rem] flex flex-col"
        :class="{ hidden: dealerStore.profileTab !== 4, block: dealerStore.profileTab == 4 }" >
        ...
        <div class="flex justify-between mt-10">
            <button class="btn btn-secondary w-32 shadow-md ml-5" @click="cancel()"> Cancel </button>
...
const cancel =()=>{
    dealerStore.alert.showMessageToast('Update Canceled')
        passwordData.value.password=''
        passwordData.value.new_password=''
        passwordData.value.confirm_password=''
        dealerStore.profileTab = 1
}
const submitPassword = () => {
    v.value.$touch()
	if (v.value.$invalid) {
        dealerStore.alert.showMessageToast(i18n.global.t('profile.input_err'))
		return
	}

    if (!Object.values(passwordData.value).includes(''))
    seller_change_password(passwordData.value).then(res => {
        dealerStore.notification.showMessageToast(i18n.global.t('profile.update_successed'))
        passwordData.value.password=''
        passwordData.value.new_password=''
        passwordData.value.confirm_password=''
        dealerStore.profileTab = 1
