<template>
    <div class="relative">
        ...
        <button 
            ...
            >
            <template class="hidden sm:block"><span class="mr-1 text-lg font-bold">+</span> Export From OrdrStartr </template>
...
const ORDR_STARTR = 'ordr_startr'

onMounted(()=>{if(store.userInfo.user_subscription.user_plan?.plugins?.[ORDR_STARTR])userGotPlugin.value = true})
...
const startWebSocketConnection =(init)=> {
  const websocket = new WebSocket(
      `${import.meta.env.VITE_APP_WEBSOCKET_URL}/ws/plugin/${ORDR_STARTR}/product/export/?token=${accessToken}`
  );
